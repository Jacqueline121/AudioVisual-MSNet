...
def make_dataset(root_path, annotation_path, gt_path, audio_path,
				 step, step_duration):
	data = read_sum_text(annotation_path)
	...
	for i in range(len(video_names)):
		...
		annot_path = os.path.join(gt_path, video_names[i])
		...
		step=int(step)
		step = 25
		for j in range(25, n_frames, step):
			sample_j = copy.deepcopy(sample)
			sample_j['frame_indices'] = list(range(max(0, j - 7), j + 1)) + list(range(j + 1, min(n_frames + 1, j + 9)))
			if len(sample_j['frame_indices']) < 16:
				break
...
class sum_db(data.Dataset):
	...
	def __getitem__(self, index):
		...
		csv_path = annot_path + '/' + video_name + '.csv'
		with open(csv_path, 'r') as f:
			reader = csv.reader(f)
			for i in reader:
				if int(i[0]) == med_indices:
					gt = int(i[1])
					break
		...
		if self.spatial_transform is not None:
			...
			self.spatial_transform_sum = copy.deepcopy(self.spatial_transform)
			del self.spatial_transform_sum.transforms[-1]
