...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.fusion4 = av_module(in_channels_dsn[3],
                                1024,
                                [temp_img_size_prev[3], temp_img_size_prev[3]],
                                temp_size_prev[3],
                                128)
        self.downsamplelayer = nn.Conv3d(1024, 512, (1, 1, 1), (2, 1, 1))
        self.bn3d = nn.BatchNorm3d(512)
        self.fuselayer = nn.Conv2d(576, 512, 3, padding=1)
        ...
        self.fc1 = fc_layer(7*7*512, 4096)
    ...
    def forward(self, x, aud):
        # aud = [64, 1, 1, 35280]
        # x = [64, 3, 16, 112, 112]
        aud = self.soundnet8(aud)  # [64, 1024, 1, 1]
        aud = [aud]
        ...
        x =torch.mul(1+att, x)  # [64, 64, 16, 56, 56]
        x = self.maxpool(x)  # [64, 64, 8, 28, 28]

        x = self.layer1(x)  # [64, 256, 8, 28, 28]
        ...
        side_out.append(tmp_)
        x = torch.mul(1+att, x)
        x = self.layer2(x)  # [64, 512, 4, 14, 14]
        ...
        side_out.append(tmp_)
        x = torch.mul(1+att, x)
        x = self.layer3(x)  # [B, 1024, 2, 7, 7]

        if self.audiovisual:
            y = self.fusion4(x, aud, crop_h, crop_w)  # [B, 64, 7, 7]
        down_x = self.downsamplelayer(x)  # [B, 512, 1, 7, 7] -- [B, 512, 7, 7]
        down_x = self.bn3d(down_x).squeeze(2)
        fushion_feature = torch.cat((down_x, y), dim=1)  # [B, 576, 7, 7]
        fushion_feature = self.fuselayer(fushion_feature)
        fushion_feature = self.bn2d(fushion_feature)
        fushion_feature = fushion_feature.view(batch, -1)
        out = self.fc1(fushion_feature)
        out = self.fc2(out)
        out = self.fc3(out)
        out = F.sigmoid(out)  # [B, 1]
...
def get_fine_tuning_parameters(model, lr, wd):
    ...
    ft_module_names_global.append('downsamplelayer')
    ft_module_names_global.append('bn3d')
    ft_module_names_global.append('fuselayer')
    ft_module_names_global.append('bn2d')
    ft_module_names_global.append('fc1')
    ft_module_names_global.append('fc2')
    ft_module_names_global.append('fc3')

    for i in range(1, 4):
        ft_module_names_global.append('layer{}'.format(i))

    ft_module_names_sum = []
    ft_module_names_sum.append('score_dsn')
    ft_module_names_sum.append('fuse')
    ft_module_names_sum.append('fuseav')

    ft_module_names_soundnet = []
    ft_module_names_soundnet.append('soundnet8')

    ft_module_names_fusion = []
    ft_module_names_fusion.append('fusion4')

    parameters = {'global':[], 'sum':[], 'other':[], 'sound':[], 'fusion':[]}
    name_parameters = {'global': [], 'sum': [], 'other': [], 'sound':[], 'fusion':[]}
    isfound = False
    for k, v in model.named_parameters():
        ...
        for ft_module in ft_module_names_fusion:
            if ft_module in k:
                if 'upscale' in k:
                    if 'weight' in k:
                        parameters['sum'].append({'params': v, 'lr': 0, 'initial_lr': 0})
                        name_parameters['sum'].append(k)
                        isfound = True
                        break
                if 'upscale_' in k:
                    if 'weight' in k:
                ...
                isfound = True
                break
        for ft_module in ft_module_names_sum:
            if ft_module in k:
                if 'side_prep' in k:
                    if 'weight' in k:
                        parameters['sum'].append({'params': v, 'weight_decay': wd, 'lr': lr, 'initial_lr': lr})
                        name_parameters['sum'].append(k)
                        isfound = True
                        break
                    elif 'bias' in k:
                        parameters['sum'].append({'params': v, 'lr': 2 * lr, 'initial_lr': 2 * lr})
                        name_parameters['sum'].append(k)
                        isfound = True
                        break
                if 'score_dsn' in k:
                    if 'weight' in k:
                        parameters['sum'].append({'params': v, 'lr': lr / 10, 'weight_decay': wd, 'initial_lr': lr / 10})
                        name_parameters['sum'].append(k)
                        isfound = True
                        break
                    elif 'bias' in k:
                        parameters['sum'].append({'params': v, 'lr': 2 * lr / 10, 'initial_lr': 2 * lr / 10})
                        name_parameters['sum'].append(k)
                        isfound = True
                        break
                if 'upscale' in k:
                ...
                if 'fuse' in k:
                    if 'weight' in k:
                        parameters['sum'].append({'params': v, 'lr': lr / 10, 'initial_lr': lr / 10, 'weight_decay': wd})
                        name_parameters['sum'].append(k)
                        isfound = True
                        break
                    elif 'bias' in k:
